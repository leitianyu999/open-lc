...
转存时只允许在平台配置的网盘临时目录根路径下创建内容，默认根路径为 `/我的资源/下载`，每次任务会创建形如 `<timestampms>-<jobid>` 的子目录。

这类限制的目的，是把一次任务产生的数据限定在受控范围内，而不是在网盘中任意扩散。该根目录被视为 Agent 管理的临时区域，空间不足重试时可能被整体清理。
...
链接生成后，相关临时文件会进入清理流程。系统会尝试删除本次任务的临时目录，删除失败时也会继续记录状态，避免把一次解析结果变成长期残留。空间不足重试时，系统可能清理整个网盘临时根目录。
...
| 创建临时目录 | `POST https://pan.baidu.com/rest/2.0/xpan/file` | `method=create`、`path`、`size=0`、`isdir=1`、`block_list=[]`、`access_token` | 在网盘临时目录根路径下创建转存目录 | `agent/api/src/baidu/service.ts`、`agent/api/src/baidu/client.ts` | <https://pan.baidu.com/union/doc/rksg0sa17> |
