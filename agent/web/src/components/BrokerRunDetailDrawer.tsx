...
export function BrokerRunDetailDrawer({ open, runId, onClose }: { open: boolean; runId: string | null; onClose: () => void }) {
  ...
  return (
    <>
      <div className={`fixed inset-0 z-40 ${open ? '' : 'pointer-events-none'}`} aria-hidden={!open}>
        ...
        />
        <aside
          className={`absolute right-0 top-0 flex h-full w-full max-w-[640px] flex-col border-l border-slate-200 bg-white shadow-2xl shadow-slate-950/20 transition-transform duration-200 max-sm:max-w-none ${open ? 'translate-x-0' : 'translate-x-full'}`}
          role="dialog"
          aria-modal="true"
          aria-label="Broker 执行详情"
        >
          <div className="flex items-start gap-3 border-b border-slate-200 px-5 py-4">
            <div className="min-w-0 flex-1">
              <h3 className="text-base font-bold text-slate-900">Broker 执行详情</h3>
              <p className="mt-1 truncate text-sm text-slate-500">{runId ? `Run ${runId}` : '未选择执行记录'}</p>
            </div>
            <button className="rounded-md p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-700" type="button" aria-label="关闭" onClick={close}>
              <X className="size-5" />
            </button>
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
            {query.isFetching && !detail ? (
              ...
              <BrokerRunDetailContent detail={detail} onOpenHistory={setSelectedRecordId} />
            ) : (
...
function BrokerRunDetailContent({ detail, onOpenHistory }: { detail: BrokerRunDetail; onOpenHistory: (recordId: number) => void }) {
  return (
      ...
      <div className="grid gap-4 rounded-lg border border-slate-200 p-4">
        ...
        {detail.localParseRecordId ? (
          <div className="flex flex-wrap items-center gap-2">
            <Button onClick={() => onOpenHistory(detail.localParseRecordId ?? 0)} size="sm" variant="secondary">
