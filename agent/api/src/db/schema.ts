...
export const baiduAccounts = sqliteTable('baidu_accounts', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  label: text('label').notNull(),
  cookie: text('cookie').notNull(),
  ownerUserId: integer('owner_user_id').references(() => users.id, { onDelete: 'set null' }),
  credentialSource: text('credential_source', { enum: ['cookie', 'open_platform'] }).notNull().default('cookie'),
  ...
  status: text('status', { enum: ['active', 'disabled', 'cooldown'] }).notNull().default('active'),
...
export const accountHealthChecks = sqliteTable('account_health_checks', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  accountId: integer('account_id').notNull().references(() => baiduAccounts.id, { onDelete: 'cascade' }),
...
export const accountStatusEvents = sqliteTable('account_status_events', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  accountId: integer('account_id').notNull().references(() => baiduAccounts.id, { onDelete: 'cascade' }),
...
export const accountTokenEvents = sqliteTable('account_token_events', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  accountId: integer('account_id').notNull().references(() => baiduAccounts.id, { onDelete: 'cascade' }),
  parseJobId: integer('parse_job_id'),
...
export const parseRecords = sqliteTable('parse_records', {
  id: integer('id').primaryKey({ autoIncrement: true }),
...
export const parseJobs = sqliteTable('parse_jobs', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  userId: integer('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  ...
  sizeBytes: integer('size_bytes').notNull(),
  md5: text('md5'),
  status: text('status', { enum: ['queued', 'running', 'success', 'failed', 'canceled'] }).notNull().default('queued'),
...
export const baiduTempFiles = sqliteTable('baidu_temp_files', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  parseRecordId: integer('parse_record_id').references(() => parseRecords.id, { onDelete: 'set null' }),
  ...
  status: text('status', { enum: ['active', 'deleted', 'delete_pending', 'delete_failed'] }).notNull().default('active'),
  errorMessage: text('error_message'),
...
export const parseEvents = sqliteTable('parse_events', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  parseRecordId: integer('parse_record_id').references(() => parseRecords.id, { onDelete: 'set null' }),
  parseJobId: integer('parse_job_id').references(() => parseJobs.id, { onDelete: 'set null' }),
  accountId: integer('account_id').references(() => baiduAccounts.id, { onDelete: 'set null' }),
...
export const brokerRuns = sqliteTable('broker_runs', {
...
export const brokerRunEvents = sqliteTable('broker_run_events', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  runId: text('run_id').references(() => brokerRuns.id, { onDelete: 'cascade' }),
  taskId: text('task_id'),
  participationId: text('participation_id'),
  type: text('type').notNull(),
  status: text('status', { enum: ['info', 'success', 'failed', 'warning'] }).notNull().default('info'),
  code: text('code'),
  message: text('message').notNull(),
  details: text('details'),
  createdAt: integer('created_at', { mode: 'timestamp_ms' }).notNull().default(sql`(unixepoch('subsec') * 1000)`),
